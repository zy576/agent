...
Web 顶部的目录浏览器属于用户显式授权面：它可以从本机根目录进入任意普通文件夹，但过滤常见凭据目录，并且不把这项能力暴露给模型。工作区重绑定与新任务准入共用同一状态锁，因此不会在工具执行中途漂移。真正切换到不同根目录时，后端同时清空旧模型历史、验证债务、运行记录和最近结果，重新从第 1 轮开始；智能体之后能调用的 `select_workspace` 仍只允许在新根目录范围内移动。
