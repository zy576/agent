...
5. 没有工具调用的 assistant 报告是软终止；最大步数和重复调用检测是硬终止。
...
源头先限流：文件、搜索和命令输出在进入历史前保留头尾并截断。达到字符预算后，`ContextManager` 永久保留 system policy 和原始用户任务，把较旧交互转换为确定性摘要，并保留最近交互。
...
- 预算：最大模型轮次、单轮工具数、总工具数和总运行秒数分别设限；超限调用会补齐 skipped tool 结果再退出，历史仍保持协议闭合。
