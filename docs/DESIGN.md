...
5. 没有工具调用的 assistant 报告是软终止；默认不设决策步数上限，工具调用总量和总运行时间同样默认不限，任务持续到模型完成报告；相同调用重复熔断负责阻止失控死循环；显式配置正数 `--max-steps`、`--max-tool-calls` 或 `--max-runtime-seconds` 时才启用对应有限终止。
...
`--subagents N` 控制最大并发子 Agent 数，范围为 `0..4`，默认 `0`，因此现有单 Agent 行为保持不变。启用后，主 Agent 在一个主任务内至多发起一批委派；批内任务必须相互独立，每个子 Agent 默认不设固定步数、只读工具调用数和时长上限，运行到写出调查报告或重复熔断为止（三者均可通过参数显式收紧）。
...
并发实现使用有界工作线程。子 Agent 的可选运行预算只在模型调用边界检查，不会抢占正在进行的 HTTP 请求；在途请求仍受单次模型请求超时约束。因此该机制提供并发和循环上界，但不是严格实时调度器或可强杀的执行隔离。
...
Web 顶部「目标工作区」与侧栏「＋」直接打开系统原生的「浏览文件夹」对话框（Windows FolderBrowserDialog，经本机后端调用），可以从任意磁盘选择任意普通文件夹作为工作区——这是用户显式授权面，浏览能力不暴露给模型。工作区重绑定与新任务准入共用同一状态锁，因此不会在工具执行中途漂移。真正切换到不同根目录时，后端同时清空旧模型历史、验证债务、运行记录和最近结果，重新从第 1 轮开始；智能体之后能调用的 `select_workspace` 仍只允许在新根目录范围内移动。

工作区与会话记录持久化在本机状态文件（默认 `~/.forgeloop/state.json`，可用 `FORGELOOP_DATA_DIR` 覆盖）：每个会话保存完整模型历史、展示对话、验证债务、轮次与最近结果；左侧栏可以新建会话或在任意会话间切换，切换时把对应工作区重新绑定并把该会话的历史装载回状态机，因此重启后历史仍然可用。状态文件损坏时降级为空库而不是拒绝启动。
...
- 子 Agent 预算：每个主任务至多一批，最多并发 4 个；每个子 Agent 默认不限步数、只读调用数和时长，可选收紧。子 Agent 只提供调查证据，不拥有主任务完成权。
