...
ForgeLoop 会只监听 `127.0.0.1`，默认选择一个可用端口并自动打开浏览器。页面左侧用于连续对话，右侧实时显示模型决策、工具调用、文件变更和验证结果。同一时间只运行一个任务；若异常回合可能已经接触工作区，会话会安全关闭，并提示重启后先检查现场。
...
ForgeLoop 默认没有模型决策步数上限，即 `max_steps=None`。它会持续工作，直到模型提交最终报告，或触发以下安全限制：

| 限制 | 默认值 | 作用 |
|---|---:|---|
| `--max-tool-calls` | `128` | 单轮任务允许的工具调用总数 |
| `--max-tool-calls-per-step` | `16` | 一次模型响应允许的工具调用数 |
| `--max-runtime-seconds` | `900` | 单轮任务最长运行时间 |
