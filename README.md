...
- **动态工作区切换**：Web 顶部的工作区选择器可以从“此电脑”进入任意磁盘，逐级浏览或直接输入绝对路径，把电脑上的任意普通文件夹设为目标工作区。切换与任务启动互斥，并会开启新的会话上下文；主智能体自身的 `select_workspace` 工具仍限定在当前工作区内。
...
顶部「目标工作区」可以点击展开本机目录浏览器：从「此电脑」选择磁盘、逐级进入或返回上级，也可以直接输入绝对路径，点「切换到此目录」即可使用该文件夹。切换时会清空上一工作区的模型历史、执行轨迹、验证债务与最近结果并开启新会话，但不会修改或删除磁盘文件；任务执行期间禁止切换。界面随系统自动切换浅色 / 深色主题，欢迎页主视觉旁的宠物点击会互动回应。
