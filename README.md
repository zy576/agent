...
- **动态工作区切换**：主智能体可以用 `list_workspaces` / `select_workspace` 工具在启动目录（工作区范围）内切换目标目录；Web 工作台顶部也有切换器，任务间隙可一键换工作区。
- **清爽双主题界面**：浅色 / 深色自动适配，整页柔和背景、右下角可拖拽点击互动的桌面宠物、欢迎页主题主视觉，桌面与移动端响应式布局。
...
ForgeLoop 会只监听 `127.0.0.1`，默认选择一个可用端口并自动打开浏览器。页面左侧用于连续对话，右侧实时显示模型决策、工具调用、文件操作和验证结果，任务完成后汇总变更文件数量。同一时间只运行一个任务；若异常回合可能已经接触工作区，会话会安全关闭，并提示重启后先检查现场。

顶部「目标工作区」可以点击展开切换器：ForgeLoop 会自动列出启动目录及其子目录里的项目目录，任务间隙可随时切换（执行中禁用；切换范围不会超出启动目录）。界面随系统自动切换浅色 / 深色主题；右下角的桌面宠物可以拖拽摆放，点击会互动回应。
...
| `list_workspaces` | 查看切换范围内的候选项目目录 | 只读，路径为范围相对路径 |
| `select_workspace` | 把活动工作区切换到范围内其他目录 | 拒绝逃逸范围、敏感目录与非目录 |

`list_workspaces` / `select_workspace` 只提供给主智能体；只读子 Agent 不包含这两个工具。切换后文件工具与命令工作目录立即指向新目录，成功事件会进入实时执行轨迹。
...
主 Agent 还可以用 `list_workspaces` 查看切换范围内的候选项目，并用 `select_workspace` 把活动工作区换到启动目录内的其他目录；切换成功会发出 `workspace_changed` 事件并进入执行轨迹。
...
- 动态工作区切换限定在启动目录（工作区范围）以内：`select_workspace` 拒绝范围外的绝对/相对路径与敏感目录，因此无法通过切换扩大文件工具边界。
