...
- **动态工作区切换**：点击顶部「目标工作区」会弹出类似 DeepSeek Harness 的文件夹选择弹窗——面包屑路径、左栏浏览右栏预览、路径直达与「此电脑」磁盘入口，可以把电脑上的任意普通文件夹设为目标工作区。切换与任务启动互斥，并会开启新的会话上下文；主智能体自身的 `select_workspace` 工具仍限定在当前工作区内。
...
点击顶部「目标工作区」会弹出文件夹选择弹窗：面包屑显示当前路径（点击任意一级可跳回）、左栏浏览当前目录、点击文件夹会在右栏预览其子目录、点铅笔图标可直接输入绝对路径回车跳转、「此电脑」回到磁盘根视图，底部「选择此文件夹」确认。切换时会清空上一工作区的模型历史、执行轨迹、验证债务与最近结果并开启新会话，但不会修改或删除磁盘文件；任务执行期间禁止切换。界面随系统自动切换浅色 / 深色主题；欢迎页以窗边图做柔和背景、肖像做主视觉，主视觉旁的宠物点击会互动回应。
