...
- **动态工作区切换**：Web 顶部的工作区选择器是一个本机目录浏览器（浏览、上级、路径直达），可以把电脑上任意文件夹设为目标工作区，就像 DeepSeek Harness 那样；主智能体自身的 `select_workspace` 工具则始终限定在当前工作区目录内。
- **清爽双主题界面**：浅色 / 深色自动适配，欢迎页主视觉旁有一个点击会互动回应的小宠物，桌面与移动端响应式布局。
...
顶部「目标工作区」可以点击展开一个本机目录浏览器：列出当前目录的子文件夹、可进入或返回上级、也可以直接在路径框输入任意路径跳转，点「选择当前目录」即可把该文件夹设为工作区。电脑上的任意文件夹都可以选（执行中禁用）；界面随系统自动切换浅色 / 深色主题，欢迎页主视觉旁的宠物点击会互动回应。
...
- 用户可以在 Web 工作台把电脑上任意文件夹设为工作区（相当于用 `--workspace` 重新指定，属于用户显式授权）；智能体自身的 `select_workspace` 工具仍被限定在当前工作区目录内，拒绝逃逸与敏感目录，因此模型无法自行扩大文件工具边界。
