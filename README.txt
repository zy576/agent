...
特色：自主维护多轮历史；模型可列出、搜索、分页读取、原子写入及精确替换文件，并以参数数组运行本地命令；工具错误会结构化回传供模型自修复；写入后必须再次验证；具备上下文压缩、API 重试、命令超时、重复循环与最大步数终止。文件路径被限制在 workspace 内，.git 与常见凭据文件不可读写，子进程仅继承最小环境变量。可用 --transcript 新建基础脱敏轨迹（不会覆盖已有文件，仍应人工检查）。
